...
contract HealthDataToken is ERC20, Ownable, ReentrancyGuard {
    struct Record {
        string ipfsHash;
    ...
    uint256 public constant REWARD_PER_RECORD = 10 * 10**18; // 10 tokens per shared record
    uint256 public constant STAKING_REWARD_RATE = 5; // 5% annual reward for staking
    ...
    event DataSharingReward(address indexed user, uint256 reward);

    constructor() ERC20("HealthDataToken", "HDT") Ownable(msg.sender) {
